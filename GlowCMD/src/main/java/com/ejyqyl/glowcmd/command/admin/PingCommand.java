...
public final class PingCommand implements CommandExecutor, TabCompleter {
    ...
    public PingCommand(@NotNull GlowCMD plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("glowcmd.ping")) {
        ...
        if (target.equals(sender)) {
            messageManager.send(sender, "ping.self", Map.of("{PING}", String.valueOf(ping)));
        } else {
            messageManager.send(sender, "ping.other", Map.of("{PING}", String.valueOf(ping), "{TARGET}", target.getName()));
        }
        return true;
    }
